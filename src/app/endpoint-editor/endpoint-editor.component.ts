...
export class EndpointEditorComponent implements OnInit {
  ...
  private availableDirections: boolean[] = [];
  ...
  ngAfterViewInit() {
    this._topService.setPbValue(60);
  ...
  setEndPoint(event) {
    ...
    if(index != -1) {
      this._boardService.level.goal = new Position(this.currentPosition[0], this.currentPosition[1], index * 2);
    }
    this._boardService.setNeedsDisplay();
  }
  ...
  private findClosestHexagon(x: number, y: number): number {
    console.log(JSON.stringify(this.coordinateX));
    console.log(JSON.stringify(this.coordinateY));
